...
<%-- <a href="<%=strBaseUrl%>&action=<%=UserServlet.ACTION_EDIT%>" title="Create a new User"><img src="<%=strImgAdminDir%>icons/icon_new_file.gif" height="20" width="20" border="0" vspace="10" alt="new file icon"></a>
<br>
--%>
...
<table>
    <tr>
        ...
        <td class="matrixHead"><a href="<%=strBaseUrl%>&sort=<%=UserSearchEngine.SORT_TYPE%>">Type</a></td>
        <td class="matrixHead"><a href="<%=strBaseUrl%>&sort=<%=UserSearchEngine.SORT_NAME%>">Name</a></td>
    ...
    while (rp.hasNext()) {
        ...
        <tr>
            ...
            <td class="<%=cssClass%>"><a href="<%=strBaseUrl%>&action=<%=UserServlet.ACTION_EDIT%>&userId=<%=thisUser.getId()%>"><%=thisUser.getActiveBillingAddress().getNameLast()%>, <%=thisUser.getActiveBillingAddress().getNameFirst()%></a></td>
